...
function InversionLens({ src, className }) {
  ...
  const setupScene = (texture) => {
    ...
    const geopmetry = new THREE.PlaneGeometry(2, 2);
    ...
    const mesh = new THREE.Mesh(geopmetry, material);
    scene.add(mesh);
    ...
    const handleResize = () => {
      if (!containerRef.current || !uniformsRef.current || !rendererRef.current)
        return;
      const width = containerRef.current.clientWidth;
      const height = containerRef.current.clientHeight;
